...
void read_angle_pid(){
  gyro.read();
  gyro_raw_data_pid = ((int)gyro.g.z); //get raw data from the z aaxis
  //gyro_raw_data = ((int)gyro.g.y); 
  gyro_dps_pid = (gyro_raw_data_pid * gyro_raw_dps_conversion_factor) / 1000; //divided by 1000 because if not it's in mdps not dps
  gyro_sum_pid += (gyro_dps_pid /50); // divided / multipled by 0.05 because thats the sampling period of 50 ms delay
  //gyro_sum += (gyro_dps / 10);
  //gyro_sum += gyro_dps;
  gyro_angle_pid = gyro_sum_pid; 
...
enum GYRO_STATES {GYRO_INIT,GYRO_START, GYRO_READ_ANGLE, GYRO_READ_ANGLE_PID} gyro_state;

void gyro_init(){
  gyro_state = GYRO_INIT;
}
void gyro_tick(){
   switch(gyro_state){ //transitions
      case GYRO_INIT:
      ...
      case GYRO_START:
        if (gyro_pid_bool == 0){
          ...
          gyro_state = GYRO_READ_ANGLE_PID;
        }        
        break;
