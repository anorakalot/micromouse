...
double kp = 0.30;//0.50,0.25,0.30,0.50,0.40,0.65
double kd = 0.20;
double ki = 0.0001;
double error;
double prev_error;
...
int sensor_45_left = 17;
int sensorReading_45_left;
int sensor_45_left_power = 6;
...
void left_turn_until() {
  ...
  while ( left_count - curr < 180 ) { // 240,210,190,100,210,170,100
    left_turn();      //85,120,110,95,90
  }
}
...
void right_turn_until() {
  ...
  while ( left_count - curr < 180 ) { //170,120 ,65,210,120
    right_turn();//113,94
  }
}
...
void reverse_turn_until() {
  ...
  while ( left_count - curr_l < 600 ) { //600,590,140,265,560
    left_turn();      //265,300,560,400,380,365,350,300
  }
...
void go_one_cell() {
  ...
  while (left_count - curr < 860) { //810,410,700
    //readIR_map();         //810,850,900,450,890,875
...
//CHOICES
bool hasfrontwall() {       //200,360,450,500,500,410
  if (sensorReading_middle > 390) { //300 , 350,370,400,275,330,360
    return true;  //360, 380
  }
  return false;
}

bool hasrightwall() {     //220,450,430,450,320,310                                //350 maybe a bit too high
  if (sensorReading_right > 290) { //100 ,500,300,250,275,400,350,310,320,343,410,420,550,500,600,500
    return true;            //NEW 320
  }
  return false;
}
...
bool hasleftwall() {     //220,340,330,320,390,370
...
//forward(base_speed,base_speed-8);
...
void readIR() {
  ...
  Serial.println("45 Readings Left right: ");
...
void pid_control() {
  ...
  */
  ...
  if (hasleftwall() != true || hasrightwall() != true) {
    motor_left = base_speed; //15,10, - 5
    motor_right = base_speed - 8 ; // - 10,-15,-1
    return;
